...
:::{.center-text}
<img src="images/parameter_estimation/normal_family.svg" width=700/>
:::
:::
...
When we treat $p(x; \theta)$ as a function of $\theta$ (instead of $x$) we call it a __likelihood__.
...
__Example (continued):__ Earlier we obtained the following log-likehood function:

$$\small{\log p(X_s; \theta) = 5\log \theta + 5\log (1-\theta) + 5\log\frac{2}{9}}.$$

The derivative of this log-likelihood function with respect to $\theta$ is equal to

$$\small{\frac{\partial}{\partial\theta}\log p(X_s;\theta) = \frac{5}{\theta} - \frac{5}{1-\theta}}.$$ 
...
ax.set_ylabel('Log-Likelihood', size = 16);
ax.set_xticks(np.arange(0, 1, 0.1))
plt.title(r'$\log p(X_s; \theta) = \log \theta + 2\log (1-\theta)$', size = 18);
plt.show()
```
...
def generate_html():
    ...
    <div class="blue-box">
        <p>
   Consider the log-likelihood function given above. Find the maximum likelihood estimate of \(\theta.\) 
       <br>
    <br>
    <br>
        <br>
...
So the maximum likelihood estimate is: $\hat{\theta} = 60.$
...
def generate_html():
    ...
    <div class="blue-box">
        <p>
   A student notices their roommate watching episode 8 of a TV series on Netflix. The show looks interesting, and the student wonders if they will have enough time to watch the entire series over a long weekend. <br>
   
   Help the student estimate the total number of episodes in the series using the maximum likelihood estimation.
       <br>
    <br>
    <br>
        <br>
...
def generate_html():
    ...
    <div class="blue-box">
        <p>
   Suppose that \(X\) follows a geometric distribution with parameter \(\theta = p,\) 
   $$P(X=x) = \theta(1-\theta)^{x-1}.$$
   The data sample has size \(n.\)<br>
