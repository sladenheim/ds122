...
display(HTML(html_content))
```
__Remark__: The mean of the exponential distribution is $\small 1/\lambda$, and the variance is $\small 1/\lambda^2$. Its CDF $\small F(x)$ is given by\
$$ \small{F(x) = P(X \leq x) = 1 - e^{-\lambda x} \mbox{ for } x \geq 0.}$$ 
:::
...
1. Given that a success has just occurred, how many trials or how long until the next success?
