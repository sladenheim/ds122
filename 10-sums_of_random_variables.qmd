...
:::{.center-text}
<img src="images/sums of random variables/covariance_table.png" width=700/> <br>
:::
```{python}
...
&= E[X] + b & \text{(definitions of expected value, PMF)}
\end{align*}}$$
...
:::{.notes}
The expected value of X times a.
:::
...
__Proof__ (discrete case): Suppose that the joint distribution of $\small{X}$ and $\small{Y}$ is $\small{p(x, y)}$. Then 
$$\small{
\begin{align*}
...
:::{.center-text}
<img src="images/sums of random variables/roulette.png" width=200/>
:::
...
We saw in an earlier lecture that the expected value of a Binomail is $Np$ so putting it all together: 
...
display(HTML(html_content))
```
__Remark__: We will prove the last two properties in HW 4.
:::
