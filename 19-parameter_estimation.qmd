...
:::{.columns}
::: {.column width="70%"}
Imagine Levels.fyi has two algorithms for predicting junior data‑scientist salaries:
:::

::: {.column width="30%"}
:::{.center-text}
<img src="images/parameter_estimation/levels.png" width=250/>
:::
:::

- **Algorithm A**: Always predicts annual salary **10k higher** than the true value.
- **Algorithm B**: On average correct, but can be **7k above or below** the true value.
...
Today, we will answer this using the ideas of bias, variance, and Mean Squared Error (MSE).
:::
...
You collect a dataset consisting of 100 swipes, each swipe from a different user. You want to estimate the probability of a right swipe, $\theta.$ 
...
:::{.center-text}
<img src="images/parameter_estimation/population_sample.png" width=750/>
:::
...
:::{style="font-size: .8em"}
In this example, the collected heights can be referred to as:
...
Parameter estimation is needed when we are given some data $\{x^{(1)}, \dots, x^{(m)}\}$ and we want to treat it as an _**independent and identically distributed (or i.i.d.)**_ sample from some population distribution.
...
:::{.center-text}
<img src="images/parameter_estimation/bias_variance_tradeoff.png" width=400/>
:::
:::
