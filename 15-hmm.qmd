...
- A concrete example from a developmental study is modelling of infant-free-play regimes. While it is not possible to directly observe if an infant is focused or exploring, a prediction can be made based on the number of toys they were interacting with during a certain period of time.
...
The __initial transition probabilities__ are typically stored in vector $\boldsymbol{\pi}.$

__Example__: For the umbrella problem, the initial probabilities are given by 

$$ \small \boldsymbol{\pi} = \begin{bmatrix} 0.6 \\ 0.4 \end{bmatrix}.$$
:::
...
__Remark__: Together $\small A, B,$ and $\small \boldsymbol{\pi}$ fully define an HMM.
...
def generate_html():
    ...
    Hidden states are ordered alphabetically, and observable states are listed as 0 followed by 1 when defining \(\boldsymbol{\pi}, A, B,\) and \(O.\)<br>
...
def generate_html():
    ...
    <div class="blue-box">
        <p>
b. Define \(\boldsymbol{\pi}, A,\) and \(B\). We always start with the healthy state, followed by the sick state, and activity level -1, followed by 0, followed by 1.
<br>
<br>
<br>
        </p>
