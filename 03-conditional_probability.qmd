...
::: {.column width="25%"}
:::{.center-text}
<img src="images/random_variables/FB_logo.png" width=120/>
:::
:::
...
These questions sound similar, but they are not. One way to see the difference is by comparing their answers. You will do just that in your next homework.
:::
...
1. _What is the chance that a person get put on Facebook's list, given that they are not a terrorist?_
...
:::{.notes}
Question 1 tells you how often innocent people get flagged.
This matters for understanding false positives, but it does not directly tell you how reliable the list is.


Question 2 tells you how likely it is that a person flagged by Facebook is actually innocent.
This is exactly the quantity that reflects whether the list is trustworthy. If this probability is high, then most people on the list are innocent, which strongly indicates that Facebook should not report the users on that list.
:::
...
::: {.column width="15%"}
:::{.center-text}
<img src="images/axioms of probability/logo.jpeg" width=200/>
:::
:::
::: {.column width="70%"}
:::{style="font-size: .7em"}
...
:::{.notes}
Notice the new column with event probability!
:::
...
$P(+) = P(+,D)+P(+,\overline D) = 0.009 + 0.099 = 0.108.$

WHY SO LOW?

The table hides the base rate of the disease, $P(D) = 0.01.$, so only 1% of the population has the disease. Of the positives, 0.009 are from that 1% and 0.099 come from the 99% who are healthy (roughly 10 times more). A 10% error rate on a large healthy group produces more positives than a 90% hit rate on a tiny sick group. This again illustrates why $P(D|+) \neq P(+|D).$
:::
...
def generate_html():
    ...
    <div class="blue-box">
        <p>
        Consider the situation that a fair coin is flipped once. Denote the event that heads come up by \(H\) and the event that tails come up by \(T\).<br> Are \(H\) and \(T\) independent? Are they disjoint?
        <br>
        <br>
        <br>
        <br>
...
2 outcomes: heads or tails. H: heads comes up, T: tails comes up
...
def generate_html():
    ...
    <div class="blue-box">
        <p>
        Can you prove the Bayes rule using the chain rule?
        </p>
    </div>
    """
html_content = generate_html()
...
::: {.notes}
Let $A be the event that the first ball is red and $B$ be the event that the second ball is red, then $P(A,B) = P(B|A)P(A) = \frac{2}{3}\frac{3}{4} = \frac{1}{2}.$

$P(A)$ is 3/4 because there are 3 red balls and 1 blue ball. $P(B|A)$ is 2/3 because after the first red ball is drawn, there are 2 red balls and 1 blue ball left.
:::
