...
**Last lecture:** We used Naive Bayes for spam classification. A quick clarification about how we modeled the text:

**What we did:** We treated each email as a sequence of words drawn from a "bag of words" (multinomial distribution). Each word is like grabbing one item from the bag.
...
**Bottom line:** You don't need to worry about the alternative approach for this class, but knowing about both may help if you encounter them in the future!
...
- MAP estimate: Just pick the highest posterior in the table (say 0.6)
- MMSE estimate: Compute weighted average of those 5 values
...
**Good news:** For conjugate priors, these integrals have known closed forms!

:::

## Summary: Two Approaches, Same Answer
...
- Works but limited to discrete grid points
- MAP/MMSE limited by granularity

**Method 2: Formula Approach**
- Capture the posterior as a formula: $p^3(1-p)$
- Can treat as continuous distribution
...
## Group Question 1: Solution (continued)
...
<b>Your Tasks:</b><br>
1. Find the formula for the prior distribution of \(\mu\) for this forest.
2. Find the formula for the likelihood of this first tree measurement.
3. Find the formula for the (unnormalized) posterior.
