...
:::{style="font-size: .8em"}
 ...
 Imagine someone randomly clicking links on the internet.
...
- The algorithm works a massive matrix of probabilities that includes the chances of moving from one page to another.

- The result? Pages that are more likely to be visited are ranked higher.
...
- Application of Markov chains for vote prediction
...
The above __state-transition diagram__ describes an example of a Markov chain.
<!-- Markov chains help us reason about what might happen many days in the future for all possible starting conditions.  -->
:::

:::{.notes}

We have created the foundation for a Markov chain.

:::

## States of a Markov Chain
:::{style="font-size: .8em"}
_**A Markov chain is a mathematical model describing a system that evolves over discrete time steps on a set of states $S$.**_ 
...
__Notation__: The state that the Markov chain is in at time $n$ is denoted as $X_n$.  Since the Markov chain evolves probabilistically, $X_n$ is a random variable.
...
:::
 ...
 __Markov property:__ Each time a Markov chain moves from the current state to the new state, the probability of that transition _**only depends on the current state.**_  Whatever states the chain was in previously do not matter.  
...
def generate_html():
    ...
    <div class="purple-box">
     <span class="label"> Markov Chain </span>
        <p>
    Given a finite set of states \(\small S \), a process \(\small X_0, X_1, \dots\) is a Markov chain on \(\small S\) if, when the process is in state \(\small j\), the probability that its next  state is \(\small i\) depends only on \(\small j\):
...
def generate_html():
    ...
    <div class="purple-box">
     <span class="label"> Transition Matrix </span>
        <p>
    The transition matrix of the Markov chain is the matrix \(\small P\) with \(\small P_{ij}  = P(X_{n+1} = i \,\vert\, X_{n} = j).\)
        </p>
    </div>
    """
html_content = generate_html()
...
:::{.center-text}
<img src="images/markov/Sunny_Rainy2.png" width=450/>
:::
...
The illustrated Markov chain with states $\small \{0, 1, 2, 3\}$ has transition matrix:
...
Now let's write the expression above using matrix notation:
...
Suppose the initial state of a Markov chain is not known with certainty, but is instead described by a _**probability distribution over possible states**_. Let this distribution be represented by the column vector $\mathbf{x}_0$, whose _**entries are nonnegative and sum to 1**_.<br>
...
That is, if we know the initial probability distribution at time 0, we can find the distribution at any later time using powers of the matrix $\small P$.
...
Then to compute the distribution two days later, i.e. $\small \mathbf{x}_2,$ we can use $\small P^2$:
