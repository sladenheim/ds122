...
def generate_html():
    ...
    <div class="purple-box">
     <span class="label">Definition: Proportionality</span>
        <p>
        We say \(f(x) \propto g(x)\) ("f is proportional to g") when there exists a constant \(c\) such that:
        <br>
        $$f(x) = c \cdot g(x)$$
...
**Why does this matter for Bayesian inference?**

- We can ignore constants in the prior and likelihood when computing the posterior, because they will be washed out in the normalizeation step.

:::


## Full vs Proportional Likelihood

:::{style="font-size: .65em"}

**Scenario:** We flip a coin 3 times and get 2 heads. We want to estimate $p$ = P(heads).

**Prior:** Uniform over $p \in \{0.2, 0.4, 0.6\}$

**Likelihood:** Binomial(k=2 | n=3, $p$)

**Full formula:** 

- $P(k=2 \mid n=3, p) = \binom{3}{2} p^2 (1-p)^1 = 3p^2(1-p)$

**Proportional formula:** 

- $P(k=2 \mid n=3, p) \propto p^2 (1-p)$

Let's compare the grid approach using both versions
...
:::{style="font-size: .58em"}

**Using full Binomial likelihood with coefficient:**

| $p$ | Prior | Likelihood = $3p^2(1-p)$ | Prior × Lik | Normalized |
|-----|-------|-------------------------|-------------|------------|
| 0.2 | 0.33 | 3(0.2)²(0.8) = 0.096 | 0.032 | 0.155 |
| 0.4 | 0.33 | 3(0.4)²(0.6) = 0.288 | 0.096 | 0.465 |
| 0.6 | 0.33 | 3(0.6)²(0.4) = 0.432 | 0.144 | 0.697 |
| | | **Sum:** | 0.207 | 1.317 |


**Using proportional likelihood (ignoring the coefficient 3):**

| $p$ | Prior | Likelihood $\propto p^2(1-p)$ | Prior × Lik | Normalized |
|-----|-------|------------------------------|-------------|------------|
| 0.2 | 0.33 | (0.2)²(0.8) = 0.032 | 0.011 | 0.155 |
| 0.4 | 0.33 | (0.4)²(0.6) = 0.096 | 0.032 | 0.465 |
| 0.6 | 0.33 | (0.6)²(0.4) = 0.144 | 0.048 | 0.697 |
| | | **Sum:** | 0.069 | 1.317 |
...
**Step 2: Combine using exponential property**
...
Expanding and then completing the square gives:
...
**Can we use conjugate priors?** Yes! Gamma-Poisson! 

(Remember when I waved my hands and said "because it makes the math work"?)
...
## Checking Our Work: Grid vs Formula

:::{style="font-size: .65em"}

Let's verify the conjugate prior formula gives the same answer as our grid approach for **France's** posterior:
...
# Grid approach (like we did before)
lambda_grid = np.linspace(0, 8, 1000)

# Prior: Gamma(1.4, 1)
prior = gamma.pdf(lambda_grid, 1.4, scale=1)

# Likelihood: observed 4 goals in 1 game, Poisson likelihood
likelihood = poisson.pmf(4, lambda_grid)

# Posterior (unnormalized)
posterior_unnorm = prior * likelihood

# Normalize
posterior_grid = posterior_unnorm / np.trapezoid(posterior_unnorm, lambda_grid)

# Formula approach: Gamma(1.4 + 4, 1 + 1) = Gamma(5.4, 2)
posterior_formula = gamma.pdf(lambda_grid, 5.4, scale=1/2)
```
...
# Left: Grid approach steps
ax[0].plot(lambda_grid, prior, ':', linewidth=2, label='Prior: Gamma(1.4, 1)', color='gray')
ax[0].plot(lambda_grid, likelihood/likelihood.max() * prior.max(), ':',
           linewidth=2, label='Likelihood (scaled)', color='orange')
ax[0].plot(lambda_grid, posterior_grid, ':', linewidth=3, label='Posterior (grid)', color='blue')
ax[0].set_xlabel('λ (goals/game)', fontsize=12)
ax[0].set_ylabel('Density', fontsize=12)
ax[0].set_title('Grid Approach: Prior × Likelihood', fontsize=13)
ax[0].legend(fontsize=10)
ax[0].grid(alpha=0.3)

# Right: Comparison
ax[1].plot(lambda_grid, posterior_grid, linewidth=3, label='Grid Posterior', color='blue', alpha=0.7)
ax[1].plot(lambda_grid, posterior_formula, '--', linewidth=3,
           label='Formula: Gamma(5.4, 2)', color='red', alpha=0.7)
ax[1].set_xlabel('λ (goals/game)', fontsize=12)
ax[1].set_ylabel('Density', fontsize=12)
ax[1].set_title('Grid vs Formula: Nearly Identical!', fontsize=13)
...
**Key insight:** The formula gives us the **exact** posterior that the grid approximates!
...
def generate_html():
    return r"""
    <div class="purple-box">
     <span class="label">Theorem: Dirichlet-Multinomial Conjugacy</span>
        <p>
...
| Multinomial | Categorical counts | Dirichlet(..., $\alpha$, ...) | Dirichlet(..., $\alpha_i + x_i$, ...) |
...
**1. Prior:** Dir(1+50, 1+30, 1+20) = **Dir(51, 31, 21)**

**2. Posterior:** Dir(51+12, 31+5, 21+3) = **Dir(63, 36, 24)**
...
3. How many 50/50 polls of size 100 would it take to sway the political analyst so their posterior mean was approximately 0.55 for Party A?<br>
        </p>
    </div>
    """
html_content = generate_html()
