...
__Question__: Do the observed fatalities follow a random pattern?
:::
...
__Example__: A canonical example is flipping a weighted coin. The coin comes up "heads" (aka "success", aka "1") with probability $p.$ If the coin is fair, $\small p=\frac{1}{2}.$ 
...
:::{style="font-size: .8em"}
The uniform distribution models the case in which all outcomes are equally probable. This distribution _**can be a discrete or continuous**_. We will introduce the continuous version of the uniform distribution in the next lecture.
