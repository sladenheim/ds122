...
## Group Question 1: Monte Carlo Integration
...
## Group Question 2: Accept-Reject Sampling
...
4. What fraction of proposals would you expect to accept? (Hint: efficiency = area under target / area under proposal)<br>
        </p>
    </div>
    """
html_content = generate_html()
...
**1. Bounding box:** $[0,1] \times [0,1]$ (since $\sqrt{1-x^2} \leq 1$ for $x \in [0,1]$)

**2. Point under curve:** A point $(x,y)$ is under the curve if $y \leq \sqrt{1-x^2}$

**3. Formula:**
$$\int_0^1 \sqrt{1-x^2} \, dx \approx 1 \times 1 \times \frac{\text{points under curve}}{\text{total points}}$$

**4. Connection to $\pi$:** This integral is the area of a quarter circle with radius 1!
...
**Problem:** Sample from $p(x) = 2x^2$ for $x \in [0,1]$ using proposal $g(x) = x$

**1. Maximum value:** $p(x) = 2x^2$ is maximized at $x=1$, so $\max p(x) = 2$

**2. Finding M:** We need $M \cdot g(x) \geq p(x)$ for all $x$
   - Need: $M \cdot x \geq 2x^2$ for all $x \in [0,1]$
   - Dividing by $x$ (for $x > 0$): $M \geq 2x$
   - Maximum at $x=1$: $M \geq 2$, so **M = 2**

**3. Algorithm steps:**
   1. Sample $x$ from $g(x) = x$ (properly normalized as $g(x) = 2x$)
   2. Sample $u \sim \text{Uniform}(0,1)$
   3. Accept if $u \leq \frac{p(x)}{M \cdot g(x)} = \frac{2x^2}{2 \cdot 2x} = \frac{x}{2}$

**4. Acceptance rate:**
   - Area under target: $\int_0^1 2x^2 \, dx = \frac{2}{3}$
   - Area under $M \cdot g(x) = 2 \cdot 2x = 4x$: $\int_0^1 4x \, dx = 2$
   - Efficiency: $\frac{2/3}{2} = \frac{1}{3}$ or **33.3%**
...
def sample_from_linear():
    """Sample from g(x) = 2x on [0,1] using inverse transform"""
    u = np.random.uniform(0, 1)
    return np.sqrt(u)  # Inverse of CDF x^2

def accept_reject_quadratic(n_samples):
    samples = []
    M = 2  # M such that M*g(x) >= p(x)
    
    while len(samples) < n_samples:
        # Sample from proposal g(x) = 2x
        x = sample_from_linear()
        # Sample uniform for accept/reject
        u = np.random.uniform(0, 1)
        # Accept if u <= p(x)/(M*g(x)) = 2x^2/(2*2x) = x/2
        if u <= x/2:
            samples.append(x)
    
    return np.array(samples)

samples = accept_reject_quadratic(5000)
print(f"Mean of samples: {np.mean(samples):.3f}")
print(f"True mean of 2x^2: {0.75:.3f}")  # E[X] = ∫ x·2x^2 dx = 3/4
```
...
**Monte Carlo methods** use random sampling to solve deterministic problems:

1. **Monte Carlo integration** estimates integrals by random sampling
2. **Pseudo-random numbers** from uniform distribution are the foundation
3. **Inverse transform sampling** converts uniform → any distribution with known inverse CDF
4. **Accept-reject sampling** works for distributions without closed-form inverse
...
**Next time:** Markov Chain Monte Carlo (MCMC) - using Markov chains to make Monte Carlo sampling even more powerful for Bayesian inference!
