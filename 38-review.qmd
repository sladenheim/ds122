...
## Solution: Bayes Table (Poisson)

:::{style="font-size: .65em"}

**Step 1: Set up the Bayes table**

**Prior:** Uniform over $\lambda \in \{2, 4, 6\}$, so $P(\lambda) = 1/3$ for each

**Step 2: Calculate likelihoods** using Poisson formula: $P(k=5 \mid \lambda) = \frac{\lambda^5 e^{-\lambda}}{5!}$

| $\lambda$ | Prior | Likelihood $P(5 \mid \lambda)$ | Prior × Lik | Posterior |
|-----------|-------|-------------------------------|-------------|-----------|
| 2 | 1/3 | $\frac{2^5 e^{-2}}{120} = \frac{32 \cdot 0.135}{120} = 0.036$ | 0.012 | 0.083 |
| 4 | 1/3 | $\frac{4^5 e^{-4}}{120} = \frac{1024 \cdot 0.018}{120} = 0.156$ | 0.052 | 0.361 |
| 6 | 1/3 | $\frac{6^5 e^{-6}}{120} = \frac{7776 \cdot 0.0025}{120} = 0.161$ | 0.054 | 0.556 |
| | | | **Sum = 0.144** | **Sum = 1.0** |

**Step 3: Answers**

- **MAP estimate:** $\lambda = 6$ (highest posterior probability)
- **MMSE estimate:** $2(0.083) + 4(0.361) + 6(0.556) = 0.166 + 1.444 + 3.336 = 4.95$ visitors/hour
...
## Solution: Bayesian Testing

:::{style="font-size: .45em"}

**1. Calculate Bayes factor:**

The Bayes factor is the ratio of likelihoods. Since the binomial coefficient cancels:

$$K = \frac{P(7H \mid H_1)}{P(7H \mid H_2)} = \frac{(0.5)^7(0.5)^3}{(0.7)^7(0.3)^3} = \frac{(0.5)^{10}}{(0.7)^7(0.3)^3}$$

$$= \frac{0.000977}{0.00223} \approx 0.438$$

Since $K < 1$, the data favors $H_2$ (biased coin).

**2. Find posterior probability of $H_1$:**

Using the fact that $P(H_1) + P(H_2) = 1$:

$$\frac{P(H_1 \mid 7H)}{P(H_2 \mid 7H)} = \frac{prior(H_1)}{prior(H_2)} \times K = \frac{0.6}{0.4} \times 0.438 = 1.5 \times 0.438 = 0.657$$

Since the two probabilities must sum to 1:

$$P(H_1 \mid 7H) = \frac{0.657}{1 + 0.657} = \frac{0.657}{1.657} \approx 0.396$$

$$P(H_2 \mid 7H) = \frac{1}{1.657} \approx 0.604$$

The data shifted our belief toward $H_2$ (the biased coin)!
...
**2. Likelihood for Positive class:**

Using Naive Bayes independence assumption:

$$P(\text{``plot" AND ``amazing"} \mid \text{Positive}) = P(\text{``plot"} \mid \text{Positive}) \times P(\text{``amazing"} \mid \text{Positive})$$
$$= 0.40 \times 0.70 = 0.28$$

**3. Likelihood for Negative class:**

$$P(\text{``plot" AND ``amazing"} \mid \text{Negative}) = P(\text{``plot"} \mid \text{Negative}) \times P(\text{``amazing"} \mid \text{Negative})$$
$$= 0.50 \times 0.10 = 0.05$$

**4. Classification (MAP):**

- Positive: $P(\text{Positive}) \times P(\text{data} \mid \text{Positive}) = 0.6 \times 0.28 = 0.168$
- Negative: $P(\text{Negative}) \times P(\text{data} \mid \text{Negative}) = 0.4 \times 0.05 = 0.020$

Since $0.168 > 0.020$, classify as **Positive**!

Posterior probability: $\frac{0.168}{0.168 + 0.020} = \frac{0.168}{0.188} \approx 0.894$ (89.4% confident it's positive)
...
Prior: Gamma(10, 2), Data: 28 events in 3 hours

Posterior: Gamma($10 + 28$, $2 + 3$) = **Gamma(38, 5)**

**2. Posterior mean:**

$$E[\lambda \mid \text{data}] = \frac{38}{5} = 7.6 \text{ visits/hour}$$

**3. Comparison to MLE:**

$$\hat{\lambda}_{\text{MLE}} = \frac{28}{3} \approx 9.33 \text{ visits/hour}$$

The Bayesian estimate is pulled toward the prior mean of 5, resulting in 7.6 (a compromise).
...
## Solution: Bayes Table Programming

:::{style="font-size: .5em"}

**Line 15 (unnormalized posterior):** `unnormalized.append(prior[i] * likelihoods[i])`

**Line 18 (normalization constant):** `total = sum(unnormalized)`

**Line 21 (normalize):** `posterior.append(u / total)`

**Line 24 (find maximum):** `map_index = posterior.index(max(posterior))`


**Line 25 (get MAP estimate):** `map_estimate = hypotheses[map_index]`
...
$p(x) = 2x$ is maximized at $x = 1$: $\max p(x) = 2$

Since $g(x) = 1$, need $M \cdot 1 \geq 2$, so $M = 2$

**2. Expected acceptance rate:**

$$\text{Rate} = \frac{1}{M} = \frac{1}{2} = 50\%$$

Alternatively: $\frac{\int_0^1 p(x) dx}{\int_0^1 M \cdot g(x) dx} = \frac{1}{2 \cdot 1} = 0.5$

**3. Acceptance probability for $x = 0.6$:**

$$\alpha = \frac{p(0.6)}{M \cdot g(0.6)} = \frac{2(0.6)}{2 \cdot 1} = \frac{1.2}{2} = 0.6$$
...
$$\alpha = \min\left(1, \frac{p(0.6 \mid \text{data})}{p(0.5 \mid \text{data})}\right) = \min\left(1, \frac{0.15}{0.20}\right) = 0.75$$

**2. Accept or reject:**

Since $u = 0.8 > \alpha = 0.75$, **reject** and stay at $\theta = 0.5$

**3. For $\theta^* = 0.55$:**

$$\alpha = \min\left(1, \frac{0.25}{0.20}\right) = \min(1, 1.25) = 1$$

Always accept (moving to higher probability).

**4. Why accept moves to lower probability:**

To properly explore the entire posterior distribution, not just the mode.
