...
1. $\frac{d}{dx} \int_a^x f(s)ds = f(x).$
...
:::{.notes}
$$\int_0^1 x^2 dx = \frac{1}{3} x^3 |_{x=0}^{x=1} =\frac{1}{3}  $$
:::
...
## Probability Density Function
:::{style="font-size: .8em"}
Continuous random variables are described by their probability density functions (PDFs). <br><br>
...
def generate_html():
    return r"""
    <div class="purple-box">
     If \(\small{F}\) is the CDF corresponding to a PDF \(\small{f},\) then \(\small{f(x) = \frac{d F(x)}{dx},}\) and \(\small{F(x) = P(X\leq x) = \int_{-\infty}^x f(s) ds.}\)
    </div>
    """
html_content = generate_html()
display(HTML(html_content))
...
:::{.center-text}
<img src="images/continuous distributions/Pdf_integral.png" width=400/>
:::
...
::: {.notes}
We are attempting to compute $F(0.5) = P(X\leq 0.5)= \int_0^{0.5} f(x) dx = \int_0^{0.5} x dx = [\frac{1}{2}x^2]_{x=0}^{x=0.5} = \frac{1}{2}(0.5)^2 - 0 = 0.125.$
:::

## Extra: Group Question 2
:::{style="font-size: .8em"}
```{python}
from IPython.core.display import HTML
...
\begin{align}
E\left[X\right]=
\int_1^{\infty}x\cdot\alpha x^{-\alpha-1}\ dx=
