...
However, many real-world variables are uncountable: they can take on any value within a range. For example, the weight of a full-term newborn often falls between 2.6 to 3.8 kg (5 lb 11 oz - 8 lb 6 oz). A baby might weigh 2.809348 kg, 3.4555329 kg, or any other value in that range.

:::{.center-text}
<img src="images/continuous distributions/newborn.png" width=300/>
...
* Short review of integral calculus
...
:::{style="font-size: .8em"}
The scores follow this _probability density function_: 
...
Question: What is the average popularity score of the news articles based on the probability density function?
:::
...
:::{style="font-size: .8em"}
A definite integral represents the area under a curve between two points on the $x$-axis.
:::
...
2. $\int_a^b f(x) dx = F(b)-F(a),$ where $F(x) = \int f(x)dx.$
...
def generate_html():
    ...
    <div class="purple-box">
     <span class="label">Probability Density Function </span>
        <p>
        If \(f\) is a probability density function describing random variable \(X\), then it must have the following properties: 
        <ol start="1">
        <li>\(\small{f(x) \geq 0,}\)</li>
        <li>\(\small{\int_{- \infty}^{\infty} f(x) dx = 1.}\)</li>
        </ol>
        </p>
    In addition, \(f\) can be used to find \( \small{P(a \leq X \leq b)} \): \(\small{P(a \leq X \leq b)= \int_a^b f(x)dx.}\)
    </div>
    """
html_content = generate_html()
display(HTML(html_content))
```
Let $F$ be the CDF corresponding to $f$, then $\small{f(x) = \frac{d F(x)}{dx},}$ and $\small{F(a) = P(X\leq a) = \int_{-\infty}^a f(x) dx.}$
...
:::{style="font-size: .8em"}
It is not difficult to see that $f(x) \geq 0$ for all values of $x$. Moreover, 
$$\int_{-\infty}^{\infty} f(x) dx =  \int_{0}^{1} 2x dx = \frac{2}{2}[x^2]_{x=0}^{x=1} = 1.$$
Hence, the provided $f$ is a PDF.
:::
...
::: {.column width="35%"}
:::{style="font-size: .8em"}
Since, for $0 \leq x \leq 1,$ $\int_{-\infty}^x f(x)dx = \int_{-\infty}^x 1 dx = x,$
$$\small{F(x) = \begin{cases}
0 & \text{ for } x < 0,\\
...
## Probability Density Function
:::{style="font-size: .8em"}
__Example__:  Suppose that $X$ is a continuous random variable with probability density function 
...
def generate_html():
    ...
    <div class="purple-box">
     <span class="label">Expected Value (Expectation) </span>
       ...
       PDF \(f,\) then \(E[X] = \int_{-\infty}^{\infty} x f(x) dx.\)
    </div>
    """
html_content = generate_html()
display(HTML(html_content))
...
def generate_html():
    ...
    <div class="purple-box">
     <span class="label">Variance </span>
        <p>
        For a continuous random variable with density function \(f(x)\) and \(\overline{X} = \mu\)
...
::: {.column width="60%"}
:::{style="font-size: .8em"}
Question: What is the average popularity score of the news articles based on the probability density function?<br>
<br>
Let us find $E[X]$ when the PDF of $X$ 
...
:::{style="font-size: .8em"}
\begin{align*} E[X] &= \int_{-\infty}^{\infty} x f(x) dx = \int_0^1 x \cdot 2x \:dx  \\
&= \int_0^1 2x^2 dx = 2 \int_0^1 x^2 dx  2 \left[ \frac{1}{3}x^3\right]_{x=0}^{x=1} = \frac{2}{3}.
\end{align*}
:::
...
def generate_html():
    ...
    <div class="blue-box">
        <p>
    Consider continuous random variable \(X\) with probability density function $$f(x) = \begin{cases} 
    \alpha x^{-\alpha-1} & \text{ for } x \geq 1,\\
    0 & \text{ otherwise. }
    \end{cases}$$
    Here, \(\alpha > 1.\) Find the expected value of \(X.\)
    <br>
    <br>
    <br>
    <br>
...
def generate_html():
    ...
    <div class="blue-box">
        <p>
    \(X\) is a continuous random variable with a cumulative distribution function (CDF) given by
    $$F(x) = \begin{cases}
