...
To answer these questions, we need  **mixture distributions** and **Poisson processes**.
...
Switching gears, remember we defined:
...
def generate_html():
    ...
    <div class="purple-box">
     <span class="label">Poisson Process</span>
        <p>
        A Poisson process models events that occur randomly over time at a constant average rate \(\lambda\) per time period.  The number of occurrences in a fixed period of time follows a Poisson distribution.
        </p>
    </div>
    """
html_content = generate_html()
